...
mod tests {
    ...
    fn test_concurrent_threadpool_kv_store_extreme_ram_usage() {
        ...
        for i in 1..=num_keys {
            ...
            pool.execute(move || {
                kv_store.lock().unwrap().set(i.to_string(), (i + 1).to_string());
            });
        }

        pool.join();

        assert_eq!(kv_store.lock().unwrap().get("100000"), Some("100001".to_string()));
        assert_eq!(kv_store.lock().unwrap().get("500000"), Some("500001".to_string()));
        assert_eq!(kv_store.lock().unwrap().get("1000000"), Some("1000001".to_string()));
    ...
    fn test_concurrent_kv_store_get_performance() {
        ...
        (1..=num_keys).into_par_iter().for_each(|i| {
            let kv_store = kv_store.clone();
            assert_eq!(kv_store.lock().unwrap().get(&i.to_string()), Some((i + 1).to_string()));
        });
    ...
    fn test_kv_store_basic_insertion() {
        ...
        let start_time = std::time::Instant::now();
        kv_store.lock().unwrap().set("key1".to_string(), "value1".to_string());

        assert_eq!(kv_store.lock().unwrap().get("key1"), Some("value1".to_string()));
        assert_eq!(kv_store.lock().unwrap().get("key2"), None);
